			...
			try
			{
				...
				while ((actualLine = reader.ReadLine()) != null)
				{
					...
					if (rawData_name.Equals(name))
					{
						...
						{
							throw new FormatException("Format invalide dans les donnees d'impact.");
						}
					}
				}

				throw new ArgumentException($"Aucun batiment avec le nom '{name}' n'a ete trouve parmi les batiments disponibles");
			}
			catch (FileNotFoundException ex)
			{
				throw new FileNotFoundException($"Impossible de trouver le fichier specifie à l'emplacement '{filePath}'", ex);
			}
		}
